...
@section('isihalaman')
    ...
    <h3>
        ...
        <h3>
            <h3>
                <center>Perpustakaan Universitas Smkn 12 Malang</center>
            </h3>
            ...
            <table class="table table-bordered table-striped" id="myTable" class="display">
                ...
                <tbody>
                    @foreach ($peminjam as $PeminjamanModel)
                        <tr>
                            {{-- <td align="center" scope="row">{{ $PeminjamanModel->firstItem() }}</td> --}}
                            <td align="center">{{ $PeminjamanModel->id_pinjam }}</td>
                            <td>{{ $PeminjamanModel->nama_petugas }}</td>
                            <td>{{ $PeminjamanModel->nama_anggota }}</td>
                            <td>{{ $PeminjamanModel->judul }}</td>
                            <td align="center">
                                <button type="button" class="btn btn-warning" data-toggle="modal"
                                    data-target="#modalPinjamEdit{{ $PeminjamanModel->id_pinjam }}">
                                    Edit
                                </button>

                                <!-- Awal Modal EDIT data Peminjaman -->
                                <div class="modal fade" id="modalPinjamEdit{{ $PeminjamanModel->id_pinjam }}" tabindex="-1"
                                    role="dialog" aria-labelledby="modalPinjamEditLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="modalPinjamEditLabel">Form Edit Data Peminjaman
                                                </h5>
                                            </div>
                                            <div class="modal-body">

                                                <form name="formpinjamedit" id="formpinjamedit"
                                                    action="/pinjam/edit/{{ $PeminjamanModel->id_pinjam }} " method="post"
                                                    enctype="multipart/form-data">
                                                    @csrf
                                                    {{ method_field('PUT') }}
                                                    <div class="form-group row">
                                                        <label for="id_pinjam" class="col-sm-4 col-form-label">ID
                                                            Pinjam</label>
                                                        <div class="col-sm-8">
                                                            <input type="text" class="form-control" id="id_pinjam"
                                                                name="id_pinjam" value="{{ $PeminjamanModel->id_pinjam }}"
                                                                readonly>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-sm-8">
                                                            <select type="text" class="form-control" id="id_petugas"
                                                                name="id_petugas">
                                                                @foreach ($petugas as $PetugasModel)
                                                                    @if ($PetugasModel->id_petugas == $p->id_petugas)
                                                                        <option value="{{ $PetugasModel->id_petugas }}"
                                                                            selected>
                                                                            {{ $PetugasModel->nama_petugas }}</option>
                                                                    @else
                                                                        <option value="{{ $PetugasModel->id_petugas }}">
                                                                            {{ $PetugasModel->nama_petugas }}</option>
                                                                    @endif
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-sm-8">
                                                            <select type="text" class="form-control" id="id_anggota"
                                                                name="id_anggota">
                                                                @foreach ($anggota as $AnggotaModel)
                                                                    @if ($AnggotaModel->id_anggota == $p->id_anggota)
                                                                        <option value="{{ $AnggotaModel->id_anggota }}"
                                                                            selected>
                                                                            {{ $AnggotaModel->nama_anggota }}</option>
                                                                    @else
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-sm-8">
                                                            <select type="text" class="form-control" id="id_buku"
                                                                name="id_buku">
                                                                @foreach ($buku as $BukuModel)
                                                                    @if ($BukuModel->id_buku == $p->id_buku)
                                                                        <option value="{{ $BukuModel->id_buku }}" selected>
                                                                            {{ $BukuModel->judul }}</option>
                                                                    @else
                                                                        <option value="{{ $BukuModel->id_buku }}">
                                                                            {{ $BukuModel->judul }}</option>
                                                                    @endif
                                                                @endforeach
                                                            </select>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <select type="text" class="form-control" id="id_buku" name="id_buku"
                                            ...
                                            <option></option>
                                            @foreach ($buku as $BukuModel)
                                                <option value="{{ $BukuModel->id_buku }}">{{ $BukuModel->judul }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
